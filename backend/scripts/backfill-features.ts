/**
 * Optimized Customer Feature Backfill
 *
 * Batches CLV lookups and processes customers efficiently
 * for large databases (22GB+).
 *
 * Run with: bun run backend/scripts/backfill-features.ts
 */
...
async function main() {
  ...
  console.log('═══════════════════════════════════════════════════');
  console.log('🧠 BACKFILLING CUSTOMER FEATURES (Optimized)');
  console.log('═══════════════════════════════════════════════════\n');
  ...
  for (let i = 0; i < total; i += BATCH_SIZE) {
    ...
    console.log(`   ${Math.min(i + BATCH_SIZE, total)}/${total} processed (${done} ok, ${failed} fail)`);
  }
...
async function backfillCustomer(db: AppDatabase, customerId: string): Promise<void> {
  ...
  // Days since last wager
  const lastWager = wagers[0]?.insert_datetime;
  const daysSince = lastWager
    ? (Date.now() - new Date(lastWager).getTime()) / 86_400_000
    : null;

  // Sharp score heuristic
  ...
  const featureJson = JSON.stringify({
    ...
    max_stake: maxStake,
    win_rate: winRate,
    sport_diversity: sportDiversity,
    large_wager_count: largeWagers,
  });

  const sourceJson = JSON.stringify({
    wager_count: wagers.length,
    agent_login: agentLogin,
    last_wager: lastWager,
  });
  ...
  await db.run(
    ...
    [
      customerId, 2, totalWagers, avgStake, maxStake, winRate, daysSince,
      sportDiversity, 0, 0, 0, sharpScore, 0, archetype, riskTier, 0,
