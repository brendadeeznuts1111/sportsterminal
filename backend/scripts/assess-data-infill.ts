/**
 * Data Infill Assessment
 *
 * Checks what customer profile data is populated vs missing.
 * Run with: bun run backend/scripts/assess-data-infill.ts
 */
...
async function main() {
  ...
  console.log('═══════════════════════════════════════════════════');
  console.log('📊 DATA INFILL ASSESSMENT');
  console.log('═══════════════════════════════════════════════════\n');
  ...
  // ─── Data Pipeline Gaps ─────────────────────────────────────────────────
  console.log('\n🔴 CRITICAL GAPS:');
  if ((totalFeatures?.count || 0) === 0) {
    console.log('   • customer_features is EMPTY — LiveFeatureService may not be running');
  }
  if ((coverage.with_snapshots || 0) === 0) {
    console.log('   • customer_snapshots is EMPTY — Buckeye customer probe not running');
  }
  if ((coverage.with_transactions || 0) === 0) {
    console.log('   • player_transactions is EMPTY — Transaction history not imported');
  }
  if ((coverage.with_deposits || 0) === 0) {
    console.log('   • deposits is EMPTY — Deposit data not imported');
  }
