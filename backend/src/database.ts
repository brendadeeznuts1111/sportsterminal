...
export async function initDatabase(url: string = dbUrl): Promise<AppDatabase> {
  ...
  `);
    ...
    CREATE TABLE IF NOT EXISTS vip_tiers (
      ...
      min_monthly_handle REAL NOT NULL DEFAULT 0,
      min_monthly_deposits REAL NOT NULL DEFAULT 0,
    ...
    CREATE TABLE IF NOT EXISTS payment_methods (
      ...
      provider TEXT NOT NULL DEFAULT 'buckeye',
      customer_id TEXT NOT NULL,
      method_type TEXT NOT NULL DEFAULT 'generic',
      handle TEXT NOT NULL,
      display_name TEXT,
      first_seen_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
      last_seen_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
      raw_json TEXT NOT NULL DEFAULT '{}',
      UNIQUE(provider, customer_id, method_type, handle)
    );
    ...
    CREATE TABLE IF NOT EXISTS player_telegram_memberships (
      player_id TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS idx_runner_tasks_status ON runner_tasks(status, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_runner_alerts_status ON runner_alerts(status, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_runner_settlements_status ON runner_settlements(status, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_support_tickets_status ON support_tickets(status, priority, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_canary_tests_status ON canary_tests(status, last_run_at DESC);
    ...
    CREATE INDEX IF NOT EXISTS idx_kyc_documents_customer ON kyc_documents(customer_id, verification_status);
    CREATE INDEX IF NOT EXISTS idx_vip_players_customer ON vip_players(customer_id);
    CREATE INDEX IF NOT EXISTS idx_payment_methods_customer ON payment_methods(customer_id, method_type);
    CREATE INDEX IF NOT EXISTS idx_payment_methods_handle ON payment_methods(handle);
    CREATE INDEX IF NOT EXISTS idx_payment_transactions_customer ON payment_transactions(customer_id, transaction_time DESC);
    CREATE INDEX IF NOT EXISTS idx_payment_transactions_method ON payment_transactions(method_type, transaction_time DESC);
    CREATE INDEX IF NOT EXISTS idx_chargebacks_customer ON chargebacks(customer_id, opened_at DESC);
    CREATE INDEX IF NOT EXISTS idx_telegram_dispatch_topic ON telegram_dispatch_log(topic, created_at DESC);
    ...
    CREATE TABLE IF NOT EXISTS ai_decisions (
      ...
      data_freshness TEXT NOT NULL CHECK(data_freshness IN ('live','cached','offline')),
    ...
    CREATE INDEX IF NOT EXISTS idx_ai_decisions_customer ON ai_decisions(customer_id, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_ai_decisions_hash ON ai_decisions(audit_hash);
    ...
    CREATE INDEX IF NOT EXISTS idx_ai_response_cache_player ON ai_response_cache(player_id, view_name, expires_at DESC);
    ...
    -- Transaction Portal: payment method catalog
    CREATE TABLE IF NOT EXISTS payment_methods (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      provider TEXT NOT NULL DEFAULT 'buckeye',
      code TEXT NOT NULL,
    ...
    -- Transaction Portal: chargeback tracking
    CREATE TABLE IF NOT EXISTS chargebacks (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      provider TEXT NOT NULL DEFAULT 'buckeye',
    ...
    CREATE INDEX IF NOT EXISTS idx_chargebacks_customer ON chargebacks(customer_id, chargeback_date DESC);
    CREATE INDEX IF NOT EXISTS idx_chargebacks_status ON chargebacks(status);

    -- Transaction Portal: soft-deleted transactions audit trail
    CREATE TABLE IF NOT EXISTS deleted_transactions (
      ...
      balance REAL,
      description TEXT,
      original_transaction_time TEXT,
      deleted_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
      deleted_by TEXT,
      reason TEXT,
      raw_json TEXT NOT NULL DEFAULT '{}'
    );
    CREATE INDEX IF NOT EXISTS idx_deleted_transactions_customer ON deleted_transactions(customer_id, deleted_at DESC);

    -- Player 360 / AI pipeline: chat session logs
    ...
    CREATE INDEX IF NOT EXISTS idx_chat_logs_session ON chat_logs(session_id, logged_at);
    CREATE INDEX IF NOT EXISTS idx_chat_logs_customer ON chat_logs(customer_id, logged_at);

    -- Player rankings: scoring heuristic cache
    ...
    CREATE INDEX IF NOT EXISTS idx_player_rankings_score ON player_rankings(composite_score DESC);
    CREATE INDEX IF NOT EXISTS idx_player_rankings_tier ON player_rankings(tier);

    -- AI Feedback: human-in-the-loop agent override trail
    CREATE TABLE IF NOT EXISTS ai_feedback (
      ...
      agent_id TEXT,
      flag_id INTEGER,
      feedback_type TEXT NOT NULL CHECK(feedback_type IN ('confirm','override','dismiss','adjust')),
    ...
    CREATE INDEX IF NOT EXISTS idx_ai_feedback_customer ON ai_feedback(customer_id, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_ai_feedback_wager ON ai_feedback(wager_number);
  `);
    ...
    try {
      ...
      console.warn('⚠️ Schema migration skipped (fresh DB):', e instanceof Error ? e.stack : e);
    }
...
export async function migrateDatabase(db: Database) {
  try {
    ...
    await db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_chargebacks_customer ON chargebacks(customer_id, opened_at DESC);
      CREATE INDEX IF NOT EXISTS idx_telegram_dispatch_topic ON telegram_dispatch_log(topic, created_at DESC);
      CREATE INDEX IF NOT EXISTS idx_telegram_dispatch_customer ON telegram_dispatch_log(customer_id, created_at DESC);
      ...
      CREATE INDEX IF NOT EXISTS idx_player_telegram_status ON player_telegram_memberships(player_id, status);
    `);
    ...
    const transactionColumns = await db.all<PragmaColumnRow>(`PRAGMA table_info(player_transactions)`);
    const transactionColumnNames = new Set(transactionColumns.map((c) => c.name));
    if (transactionColumns.length > 0 && !transactionColumnNames.has('payment_method')) {
      await db.exec(`ALTER TABLE player_transactions ADD COLUMN payment_method TEXT`);
      console.log('Migration: added payment_method to player_transactions');
    }
    ...
    const telegramDispatchColumns = await db.all<PragmaColumnRow>(`PRAGMA table_info(telegram_dispatch_log)`);
    if (!telegramDispatchColumns.some((col) => col.name === 'customer_id')) {
      await db.exec(`ALTER TABLE telegram_dispatch_log ADD COLUMN customer_id TEXT`);
      console.log('Migration: added customer_id to telegram_dispatch_log');
    }
    ...
    await db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_ai_decisions_hash ON ai_decisions(audit_hash);
    `);
    console.log('📊 Migration: ensured ai_decisions table');
    ...
    await db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_ai_response_cache_player ON ai_response_cache(player_id, view_name, expires_at DESC);
    `);
    console.log('📊 Migration: ensured ai_response_cache table');
...
async function rebuildAgentClosureMigration(db: Database): Promise<void> {
  const row = await db.get<{ c: number }>(`SELECT COUNT(*) as c FROM agent_hierarchy WHERE provider = 'buckeye'`);
  if (!row || row.c === 0) {
    console.log('  Closure rebuild skipped: agent_hierarchy is empty');
    return;
  }
...
async function backfillWagerHierarchyMigration(
  ...
  const row = await db.get<{ c: number }>(`SELECT COUNT(*) as c FROM ${tableName}`);
  if (!row || row.c === 0) {
    console.log(`  Backfill skipped: ${tableName} is empty`);
    return;
  }
  await db.run(`
    UPDATE ${tableName}
