...
describe('system status health route', () => {
  ...
  test('groups repeated Player 360 Buckeye auth failures into one operator issue', async () => {
    ...
    const body = await response!.json() as SystemStatusBody;
    const authIssues = body.issues.filter((issue) => issue.title.includes('Buckeye auth'));
    ...
    expect(body.issues.some((issue) => issue.detail.includes('parse error'))).toBe(true);
  });
